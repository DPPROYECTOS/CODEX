...
export let supabaseMaster: SupabaseClient | null = null; 
...
export const appwriteService = {
  ...
  async login(email: string, password?: string, forceDeviceReset?: boolean): Promise<User> {
    if (!password) {
      ...
      try {
        ...
        if (!authorizedIds.includes(currentDeviceId)) {
            ...
            if (authorizedIds.length >= maxAllowed) {
                if (forceDeviceReset) {
                    // Borrar vinculaciones previas del usuario
                    await supabase.from('user_authorized_ips').delete().eq('user_id', authData.user.id);
                    // Insertar la nueva terminal actual
        ...
        return {
            ...
            hierarchy: profileData?.hierarchy || 'Personal Administrativo'
        };
    ...
    if (isLegacyReady() && supabase) {
      try {
        ...
        return {
            ...
            isAdminHardware: isAdminHardware || isFullAdmin,
            hierarchy: profile?.hierarchy || 'Personal Administrativo'
        };
      } catch (e) { return null; }
    }
    return null;
    ...
    try {
        ...
        channel
            .on('presence', { event: 'sync' }, () => {
                ...
                // Agrupar/Deduplicar por userId y dispositivo (ip)
                const groupedUsers: { [uniqueKey: string]: OnlineUser } = {};
                rawUsers.forEach(u => {
                    const uniqueKey = `${u.userId}-${u.ip}`;
                    if (!groupedUsers[uniqueKey]) {
                        groupedUsers[uniqueKey] = { ...u };
                    } else {
                        groupedUsers[uniqueKey].connectionsCount = (groupedUsers[uniqueKey].connectionsCount || 1) + 1;
                        // Conservar la marca de tiempo más reciente de conexión
                        if (new Date(u.onlineAt) > new Date(groupedUsers[uniqueKey].onlineAt)) {
                            groupedUsers[uniqueKey].onlineAt = u.onlineAt;
                        }
                        if (u.deviceSpecs && !groupedUsers[uniqueKey].deviceSpecs) {
                            groupedUsers[uniqueKey].deviceSpecs = u.deviceSpecs;
                        }
                    }
                });

                onSync(Object.values(groupedUsers));
            });
  ...
  async adminUpdateUserHierarchy(userId: string, hierarchy: string): Promise<boolean> {
    if (isLegacyReady() && supabase) {
        try {
            const { error } = await supabase.from('users').update({ hierarchy }).eq('id', userId);
            return !error;
        } catch (e) {
            console.error("Error updating user hierarchy", e);
            return false;
        }
    }
    return false;
