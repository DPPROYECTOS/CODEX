...
export const Layout: React.FC = () => {
  ...
  const isSuperAdmin = user?.email && SUPER_ADMINS.some(email => email.toLowerCase() === user.email.toLowerCase());
  
  if (isSuperAdmin) {
    links.push({ to: '/admin/chat', icon: MessageSquare, label: 'Chat Administrativo' });
  }
      ...
      <aside className={`hidden md:flex w-64 bg-slate-950 text-white flex-col fixed h-full z-10 shadow-2xl border-r border-white/10 ${user.isImpersonating ? 'mt-10' : ''}`}>
        <div className="p-6 border-b border-white/10">
          <h1 className="text-2xl font-black tracking-tight flex items-center gap-2">
            CODEX 
            <span className="text-indigo-300 font-bold text-[10px] bg-indigo-900/50 px-2.5 py-0.5 rounded-full border border-indigo-500/30">v1.7</span>
          </h1>
             ...
             <h2 className="text-2xl font-bold text-white">
               {location.pathname === '/' ? 'Panel Principal' : 
                ...
                location.pathname === '/admin/proposals' ? 'Gestión de Mejoras' : 
                 location.pathname === '/admin/chat' ? 'Chat Administrativo' : ''}
             </h2>
