...
export const AdminPanel: React.FC = () => {
  ...
  // Sorting & Filtering for user hierarchy
  const [searchTerm, setSearchTerm] = useState('');
  const [sortByHierarchy, setSortByHierarchy] = useState<'ASC' | 'DESC' | 'NONE'>('ASC');
  const [updatingHierarchyId, setUpdatingHierarchyId] = useState<string | null>(null);
  ...
  const handleUpdateHierarchy = async (userId: string, newHierarchy: string) => {
      setUpdatingHierarchyId(userId);
      const success = await appwriteService.adminUpdateUserHierarchy(userId, newHierarchy);
      if (success) {
          setActionMessage(`Jerarquía actualizada a ${newHierarchy}.`);
          setUsersList(prev => prev.map(u => u.$id === userId ? { ...u, hierarchy: newHierarchy } : u));
          setTimeout(() => setActionMessage(null), 4000);
      } else {
          alert("Error al actualizar la jerarquía.");
      }
      setUpdatingHierarchyId(null);
  };
  ...
  const getHierarchyWeight = (h?: string) => {
    if (!h) return 5;
    return HIERARCHY_WEIGHTS[h] || 5;
  };
  ...
  const filteredAndSortedUsers = usersList
    .filter(u => {
        ...
        return (
            ...
            (u.signedName || '').toLowerCase().includes(query) ||
            (u.email || '').toLowerCase().includes(query) ||
            (u.area || '').toLowerCase().includes(query) ||
            (u.hierarchy || '').toLowerCase().includes(query)
        );
    })
    .sort((a, b) => {
        if (sortByHierarchy === 'ASC') {
            const diff = getHierarchyWeight(a.hierarchy) - getHierarchyWeight(b.hierarchy);
            if (diff !== 0) return diff;
            return (a.name || '').localeCompare(b.name || '');
        } else if (sortByHierarchy === 'DESC') {
            const diff = getHierarchyWeight(b.hierarchy) - getHierarchyWeight(a.hierarchy);
            if (diff !== 0) return diff;
            return (a.name || '').localeCompare(b.name || '');
        } else {
            return (a.name || '').localeCompare(b.name || '');
  ...
  return (
    <div className="space-y-6">
                                     ...
                                     <div>
                                         <div className="flex items-center gap-2">
                                             ...
                                             {u.connectionsCount && u.connectionsCount > 1 && (
                                                 <span className="text-[9px] font-black font-mono bg-blue-900/40 text-blue-300 border border-blue-500/30 px-1.5 py-0.5 rounded tracking-tighter uppercase animate-pulse">
                                                     {u.connectionsCount} Pestañas
                                                 </span>
                                             )}
                                         </div>
      ...
      <div className="bg-slate-900 rounded-xl shadow-sm border border-white/10 overflow-hidden mt-8">
        ...
        {/* Panel de Filtros y Ordenamiento por Jerarquía */}
        <div className="p-4 bg-slate-950/40 border-b border-white/10 flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="relative w-full md:w-80">
                <input 
                    ...
                    className="w-full bg-slate-950 border border-white/10 rounded-lg py-2 px-3 pl-9 text-xs text-gray-200 placeholder-gray-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                />
            ...
            <div className="flex gap-3 items-center w-full md:w-auto justify-end">
                ...
                <select 
                    value={sortByHierarchy} 
                    onChange={(e) => setSortByHierarchy(e.target.value as any)}
                    className="bg-slate-950 border border-white/10 text-gray-200 text-xs font-bold rounded-lg p-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none cursor-pointer"
                >
                    <option value="ASC">Jerarquía: Alta a Baja (Director → Colaborador)</option>
                    <option value="DESC">Jerarquía: Baja a Alta (Colaborador → Director)</option>
            ...
            <table className="w-full text-sm text-left">
                <thead className="bg-slate-950 text-gray-400 font-medium border-b border-white/10">
                    <tr>
                        ...
                        <th className="px-6 py-4">Jerarquía / Cargo</th>
                ...
                <tbody className="divide-y divide-white/5">
                    {loading ? (
                        <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-500">Cargando datos...</td></tr>
                    ) : filteredAndSortedUsers.length === 0 ? (
                        <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-500">No se encontraron usuarios con los criterios de búsqueda.</td></tr>
                    ) : (
                        filteredAndSortedUsers.map((u) => (
                            <tr key={u.$id} className="hover:bg-white/5 transition-colors group/row">
                                ...
                                <td className="px-6 py-4">
                                    <select 
                                        value={u.hierarchy || 'Personal Administrativo'} 
                                        onChange={(e) => handleUpdateHierarchy(u.$id, e.target.value)}
                                        className="bg-slate-950 border border-white/10 text-gray-200 text-xs font-bold rounded-lg p-1.5 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none uppercase tracking-wide cursor-pointer hover:bg-slate-900 transition-colors"
                                        disabled={updatingHierarchyId === u.$id}
                                    >
