          ...
          <div className="flex items-center gap-3">
            ...
            <div>
                <div className="flex items-center gap-1.5">
                    <h4 className="text-sm font-black text-gray-200 uppercase tracking-tighter truncate max-w-[120px]">{user.name}</h4>
                    {user.connectionsCount && user.connectionsCount > 1 && (
                        <span className="text-[8px] font-black font-mono bg-blue-900/40 text-blue-300 border border-blue-500/25 px-1 py-0.2 rounded tracking-tighter uppercase shrink-0">
                            {user.connectionsCount}T
                        </span>
                    )}
                </div>
    ...
    <div className="space-y-8 animate-in fade-in duration-500 font-mono">
      ...
      {onlineUsers.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {onlineUsers.map((u, idx) => <TelemetryCard key={`${u.userId}-${idx}`} user={u} />)}
        </div>
      )}
