...
export const Login: React.FC = () => {
  ...
  const [showDeviceResetBtn, setShowDeviceResetBtn] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent, forceReset = false) => {
    ...
    setShowDeviceResetBtn(false);
    try {
      await login(email, password, forceReset);
      saveEmailToHistory(email);
    } catch (err: any) {
      console.error(err);
      const errMsg = err.message || 'Error desconocido.';
      setError(errMsg);
      if (errMsg.toUpperCase().includes('LIMITE DE DISPOSITIVOS') || errMsg.toUpperCase().includes('EQUIPOS VINCULADOS')) {
        setShowDeviceResetBtn(true);
      }
    } finally {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#020617] px-4 py-8 relative overflow-hidden">
                ...
                <div className="flex-1">
                    ...
                    {showDeviceResetBtn && (
                      <button
                        ...
                        className="mt-3 w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg transition-all border border-indigo-400/30 cursor-pointer shadow-lg active:scale-[0.98]"
                      >
                        Desvincular otros equipos y vincular este terminal
                      </button>
                    )}
                </div>
            </div>
      ...
      <div className="absolute bottom-6 right-6 select-none opacity-40 hover:opacity-90 transition-opacity">
          <span className="text-[9px] font-black uppercase tracking-[0.2em] text-indigo-300 bg-indigo-950/60 px-3 py-1.5 rounded-full border border-indigo-500/30 shadow-lg shadow-indigo-950/50 backdrop-blur-sm">
              Build v1.7
          </span>
      </div>
    </div>
