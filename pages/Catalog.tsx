...
export const Catalog: React.FC = () => {
  ...
  const userAllowedAreas = user?.allowedAreas || [];
  const isSingleAreaUser = !hasGlobalVisibility && userAllowedAreas.length === 1;
  ...
  useEffect(() => {
      if (isSingleAreaUser && !selectedArea && userAllowedAreas[0]) {
          setSelectedArea(userAllowedAreas[0]);
      }
  }, [isSingleAreaUser, selectedArea, userAllowedAreas]);

  const displayAreas = availableAreas.filter(area => {
      if (hasGlobalVisibility) return true;
      const normalizedArea = normalizeString(area);
      return userAllowedAreas.some(allowed => normalizeString(allowed) === normalizedArea);
  });
  ...
  const { visibleFolders, visibleDocs } = useMemo(() => {
    ...
    if (currentFolder) {
        ...
        docs = filteredByStatus.filter(p => p.folder_id && String(p.folder_id) === String(currentFolder.id));
    } else {
        ...
        docs = filteredByStatus.filter(p => !p.folder_id || String(p.folder_id).trim() === "");
        ...
        const requiredFolderIds = new Set<string>(
            filteredByStatus
                .filter(p => p.folder_id && String(p.folder_id).trim() !== "")
                .map(p => String(p.folder_id).trim())
        );

        // 3. Cruzar con las carpetas que tenemos en DB
        const catalogFolders = areaFolders.filter(f => requiredFolderIds.has(String(f.id)));
        // Fix: Explicitly type Set to string to avoid 'unknown' type errors
        const catalogIds = new Set<string>(catalogFolders.map(f => String(f.id)));
        ...
        requiredFolderIds.forEach((id: string) => {
            if (!catalogIds.has(String(id))) {
                missingFolders.push({
                    id,
                    name: `SECCIÓN ADICIONAL (${String(id).substring(0, 5).toUpperCase()})`,
                    parent_id: null,
  ...
  const getFolderCount = (folderId: string) => filteredByStatus.filter(p => p.folder_id && String(p.folder_id) === String(folderId)).length;
